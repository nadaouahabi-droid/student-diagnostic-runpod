...
Serves the HTML frontend and proxies requests to RunPod.
Keeps the RunPod API key server-side (never exposed to browser).
"""
...
def run_job():
    """Submit a job to RunPod and return immediately with job ID."""
...
def job_status(job_id):
    """Poll RunPod for job status/output."""
    try:
        r = requests.get(
...
def run_sync():
    """Submit a job and wait for result (for fast actions like health)."""
    payload = request.get_json()
    try:
        r = requests.post(
